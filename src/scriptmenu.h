/*
	...
	BlackjackCheat use. Only change from the original: MenuSwitchPressed()
	below uses VK_F12 instead of VK_F5 -- rebound from the originally
	vendored VK_F10 because PokerCheat's own menu already uses F10 and
	BlackjackCheat's already uses F11 (both siblings under the same "RDR2
	Shit" folder, plausibly loaded into the game at the same time).
*/
...
{
public:
	// Toggle key F12 -- see this file's own header comment for why (F10/F11
	// are already claimed by PokerCheat/BlackjackCheat's own menus).
	static bool MenuSwitchPressed()
	{
		return IsKeyJustUp(VK_F12);
	}

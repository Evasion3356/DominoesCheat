/*
	...
	scrThread pool -- see GamePointers.h). Vendored unchanged from
	PokerCheat/BlackjackCheat's own PatternScan.h -- generic, no
	dominoes-specific content at all.

	Pattern syntax: space-separated hex bytes, "?" for a wildcard byte, e.g.
	"48 8D 0D ? ? ? ? E8 ? ? ? ? EB 0B 8B 0D" (HorseMenu's own
	"ScriptThreads&RunScriptThreads" signature).
*/
...
namespace PatternScan
{
	...
	// to the absolute address it targets -- standard x64
	// `LEA reg, [rip+disp32]` / `CALL [rip+disp32]` encoding.
